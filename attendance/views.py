...
from .forms import EmployeeForm, EmployeeEditForm, ChangePasswordForm
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                messages.error(request, f'Password reset, but email could not be sent: {mail_error}')
...
def admin_dashboard(request):
    ...
    # Recent attendance
    recent_attendance = Attendance.objects.select_related('employee__user').order_by('-date', '-check_in_time')[:20]
    ...
    # Employees list
    employees = Employee.objects.filter(is_active=True).select_related('user')[:10]
        ...
        if form.is_valid():
            try:
                ...
                try:
                    ...
                    messages.warning(request, f'Account created, but welcome email could not be sent: {mail_error}')
                ...
                if captured_photo and not request.FILES.get('photo'):
                    ...
                    if encoding is not None and len(encoding) > 0:
                        ...
                        employee.save()
                        messages.success(request, 'Employee added & face encoded!')
                    else:
                ...
                #  No photo
                    ...
                    employee.save()
                    messages.success(request, f'Employee {employee.employee_id} added. Please add a photo.')
...
def export_attendance_csv(request):
    ...
    writer = csv.writer(response)
    writer.writerow(['Employee Name', 'Employee ID', 'Department', 'Date', 'Check In Time', 'Check Out Time', 'Status', 'Marked By'])
...
def export_attendance_pdf(request):
    ...
    # Table data
    data = [['Employee Name', 'ID', 'Department', 'Date', 'Check In', 'Check Out', 'Status', 'Marked By']]
    ...
    # Create table
    col_widths = [140, 70, 80, 70, 70, 70, 80, 90]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
